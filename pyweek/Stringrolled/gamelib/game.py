...
def game(mygame, level, graphics):
    ...
    while game_quit == 0:
        ...
        if gamepad.left == 0:
            ...
            gamepad.right = 0;
        gamepad.up = keys[pygame.K_UP] or keys[pygame.K_z]
        gamepad.down = keys[pygame.K_DOWN]
        if keys[pygame.K_TAB] and gamepad.toggle != 0:
            gamepad.toggle = 2
        else:
            gamepad.toggle = keys[pygame.K_TAB]
        if (keys[pygame.K_UP] or keys[pygame.K_z]) and gamepad.jump != 0:
            gamepad.jump = 2
        else:
            gamepad.jump = keys[pygame.K_UP] or keys[pygame.K_z]

        if (keys[pygame.K_SPACE] or keys[pygame.K_x]) and gamepad.throw != 0:
            gamepad.throw = 2
        else:
            gamepad.throw = keys[pygame.K_SPACE] or keys[pygame.K_x]      
